...
export function hexPulse() {
  ...
  let cRect: ReturnType<typeof c.getBoundingClientRect>;
  ...
  function setSizes() {
    max = Math.max(innerHeight, innerHeight);
    mouse.r = max / 2.6;
  ...
  function render() {
    ...
    shapes.forEach((shape) => {
      ...
      drawHexagon(
        shape.x,
        shape.y,
        shape.size * shape.scale * shape.scale2,
        shape.color
      );
    });
  }

  function getDistance(shape: Shape) {
  ...
  c.addEventListener("pointerleave", () =>
    pointerMove({ x: innerWidth / 2, y: innerHeight / 2 })
  );
